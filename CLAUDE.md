...
**当前阶段：v0.1/v0.2/v0.3/v0.4 已发版（git tag v0.1/v0.2/v0.3/v0.4）。v0.5 SEO 工具 + CRM 图片开发完成待验收（扒图部分"几乎不能用"移出重做）。数据底层逻辑五层模型已定稿（2026-09-03，`docs/数据底层逻辑-实物物料卖法三层模型.md`）。v0.6 扒图重做开发完成 + 集成验收真跑通过 + 批 6 复核反馈落盘改造完成 + 批 7 夸克上传开发完成（2026-09-03，详设 `docs/详设-v0.6-扒图重做.md` 已确认，5 批开发 + B3 逐批验收全通过 + 批 6 复核反馈（迁移 0012 netdisk 三列/一链接一文件夹 meta.txt/zip 移除）+ 批 7 夸克上传（quark connector + 上传执行点 + 补传链 + 设置页登录块 + A70-A72），A1-A69 全绿 + A70-A72 implemented（全量 879 passed 五绿），待主代理验收批 7 + 批 8 队列迁移收口 → 用户复核（真实分享链接冒烟 + 真实夸克授权 manual 项）**，版本规划已重做（2026-09-03，`docs/版本规划.md`：v0.5 SEO → v0.6 扒图重做 → v0.7 SKU → v0.8 卖法/SEO → v0.9 上架/ERP → v1.0 权限+收口）。**路线**：需求分析 ✅ → 概设 ✅ → 详设 ✅（v0.1/v0.2/v0.3/v0.4/v0.6）→ 页面原型 ✅ → 开发 ✅（v0.1/v0.2/v0.3/v0.4/v0.6）→ 测试 ✅（A1-A72）→ 集成验收 ✅（v0.6）→ 复核 ✅（v0.1/v0.2/v0.3/v0.4 tag）→ v0.5 开发完成待验收 → v0.6 扒图重做（批 7 完成待主代理验收批 7/批 8 + 用户复核）。
