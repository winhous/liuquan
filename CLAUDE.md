...
**当前阶段：v0.1 全部完成（2026-08-28：四绿全真跑、验收断言 A1-A11 全绿（含真模型冒烟）、单测 339 通过 + 代码审查修复；待用户复核后 git tag v0.1）。v0.2 页面原型完成（整体系统框架 + Tabler 换装 + 导航按用户定方案，运行于 http://127.0.0.1:8090）；TM 详设待撰写（业务细则已拍板：无优先级分级[决策 11 修订]/全部提案人工审/统计必有[决策 15]）。** 路线：需求分析 ✅ → 概设 ✅ → 详设 ✅（v0.1）→ 页面原型 ✅（v0.2 框架）→ 开发 ✅（v0.1）→ 测试 ✅（A1-A11）→ 用户复核（待 tag）→ v0.2 详设/开发。
