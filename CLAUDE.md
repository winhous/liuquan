...
**当前阶段：v0.1/v0.2/v0.3 已发版（git tag v0.1/v0.2/v0.3）。v0.4 定时闭环 + 设置页（配置地基）开发完成（2026-09-01：四批开发 + 集成验收真跑通过，746 passed 四绿全绿，A1-A45 全绿），待用户复核后 git tag v0.4。** 路线：需求分析 ✅ → 概设 ✅ → 详设 ✅（v0.1/v0.2/v0.3/v0.4）→ 页面原型 ✅ → 开发 ✅（v0.1/v0.2/v0.3/v0.4）→ 测试 ✅（A1-A11 + A12-A26 + A27-A37 + A38-A45）→ 集成验收 ✅ → 复核 ✅（v0.1/v0.2/v0.3 tag）→ v0.4 复核。v0.5 SEO + 扒图 / v0.6 ERP 延后不排期（决策 33），v0.7 权限系统。
