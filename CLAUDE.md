...
**当前阶段：v0.1 已发版（git tag v0.1，2026-08-28 用户复核通过）。v0.2 开发完成（2026-08-28：TM 详设确认后 T1-T8 全部落地——业务库 tm schema 三表/引擎三接口常驻 server/转交器落库/演示链端到端/页面接真数据/测试 472 条/A12-A26 验收断言全绿 + 集成验收真跑通）；待用户复核后 git tag v0.2。** 路线：需求分析 ✅ → 概设 ✅ → 详设 ✅（v0.1/v0.2）→ 页面原型 ✅ → 开发 ✅（v0.1/v0.2）→ 测试 ✅（A1-A11 + A12-A26）→ 集成验收 ✅ → v0.2 用户复核（待 tag）。
