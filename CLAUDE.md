...
**当前阶段：v0.1 全部完成（2026-08-28：四绿全真跑、验收断言 A1-A11 全绿（含真模型冒烟）、单测 339 通过 + 代码审查修复；待用户复核后 git tag v0.1）。v0.2 页面原型完成（整体系统框架 + Tabler 换装 + 导航按用户定方案，运行于 http://127.0.0.1:8090）；TM 详设已撰写（`docs/详设-v0.2-TM.md`，2026-08-28）待用户确认后进 v0.2 开发。** 路线：需求分析 ✅ → 概设 ✅ → 详设 ✅（v0.1）→ 页面原型 ✅（v0.2 框架）→ 开发 ✅（v0.1）→ 测试 ✅（A1-A11）→ 用户复核（待 tag）→ v0.2 详设确认/开发。
...
- `docs/详设-v0.2-TM.md` - v0.2 详设（tm schema 三表字段级/提案审核流/引擎三接口/双库迁移/演示链，2026-08-28 撰写，待用户确认）
