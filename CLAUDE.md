...
**当前阶段：v0.1/v0.2/v0.3/v0.4/v0.6 已发版（git tag v0.1/v0.2/v0.3/v0.4/v0.6）；**v0.7 SKU/货档案建档「复核修订 v3」全部完成，待用户复核 → git tag v0.7**（2026-09-04：用户首版复核 8 条问题全部修复——归 ERP 导航（children=货品建档/库存管理/补货建议/异常预警/生产库存只读）、编号自动生成（类型前缀 P/C/U+商品代号+序号，在售不编入号）、规格属性 specs（迁移 0014）、建档后可编辑、建档页去库存（库存归 ERP 库存管理页）、图库文件夹浏览器+标签搜索+上传入口、BUG-3 建档提交 500 已修复；批 1 服务层+批 2 页面层+批 3 整体独立测试（11/12+2 bug 修复闭环）全绿，A76-A95 全绿、907+ passed 五绿无 skipped，dev 库迁移 head=0014，双服务运行 v0.7.1 代码；复核走查 9 项见 `docs/复核报告-v0.7.md`）**，版本规划已重做（2026-09-03，`docs/版本规划.md`：v0.5 SEO → v0.6 扒图重做 → v0.7 SKU → v0.8 卖法/SEO → v0.9 上架/ERP → v1.0 权限+收口）。**路线**：需求分析 ✅ → 概设 ✅ → 详设 ✅（v0.1/v0.2/v0.3/v0.4/v0.6）→ 页面原型 ✅ → 开发 ✅（v0.1/v0.2/v0.3/v0.4/v0.6）→ 测试 ✅（A1-A72）→ 集成验收 ✅（v0.6）→ 复核 ✅（v0.1/v0.2/v0.3/v0.4 tag）→ v0.5 开发完成待验收 → v0.6 扒图重做（批 7 完成待主代理验收批 7/批 8 + 用户复核）。**2026-09-04 晚：v0.7 复核挂起，并入「全系统原型重构轮」**（产品经理用户判定现页面原型/功能设计/交互不足：视觉/交互/一致性/流程四类问题；决定：先基于现有系统彻底规划高保真网页原型 → 按原型出每页详细需求文档 → 再开发，流程补「原型确认门」。产出：`docs/原型盘点-全系统页面与交互问题.md` + 高保真样张 `docs/prototype/web/`（p1 建档列表 / p2 新建档案，Tabler 同款高保真，静态服务 127.0.0.1:8091）；Figma 路线判定死路：官方无写画布 API + 本机云 API 不通（代理仅通网页））。
