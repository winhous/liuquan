...
**当前阶段：v0.1/v0.2/v0.3/v0.4 已发版（git tag v0.1/v0.2/v0.3/v0.4）。v0.5 SEO 工具 + CRM 图片开发完成待验收（扒图部分"几乎不能用"移出重做）。数据底层逻辑五层模型已定稿（2026-09-03，`docs/数据底层逻辑-实物物料卖法三层模型.md`）。下个版本 = v0.6 扒图重做（素材库落地）**，版本规划已重做（2026-09-03，`docs/版本规划.md`：v0.5 SEO → v0.6 扒图重做 → v0.7 SKU → v0.8 卖法/SEO → v0.9 上架/ERP → v1.0 权限+收口）。**路线**：需求分析 ✅ → 概设 ✅ → 详设 ✅（v0.1/v0.2/v0.3/v0.4）→ 页面原型 ✅ → 开发 ✅（v0.1/v0.2/v0.3/v0.4）→ 测试 ✅（A1-A11 + A12-A26 + A27-A37 + A38-A45）→ 集成验收 ✅ → 复核 ✅（v0.1/v0.2/v0.3/v0.4 tag）→ v0.5 开发完成待验收 → v0.6 扒图重做（详设待出）。
