...
**当前阶段：v0.1 已发版（git tag v0.1）、v0.2 已发版（git tag v0.2，2026-08-28 用户复核通过）。v0.3 CRM 平移详设已确认（docs/详设-v0.3-CRM.md，2026-08-28），开发进行中（三批子代理任务，主代理逐批四绿验收 + A27-A37）。** 路线：需求分析 ✅ → 概设 ✅ → 详设 ✅（v0.1/v0.2/v0.3）→ 页面原型 ✅ → 开发 ✅（v0.1/v0.2）→ 测试 ✅（A1-A11 + A12-A26）→ 集成验收 ✅ → 复核 ✅（v0.1/v0.2 tag）→ v0.3 开发中。
