...
- 匹配方式为子串包含（term in 常量/标量值）：对业务词零容忍侧保守，
  宁可误伤不放过；词表从 config/ 自动生长后若噪声过大，可在此收紧
- 自动词表过滤：布尔/None 值不入表（不是业务规格专名，且 isinstance
  (True, int) 陷阱需先判 bool）；长度 < 2 的词不入表（防单字符噪声，
  详设 §9 样例「13 个标签」为两位数仍入表）；config 的键与值都入表
...
def _extract_terms(data: object) -> set[str]:
    """config yaml 数据的键与标量值 -> 词表词集合（布尔/None/短词过滤）。"""
    terms: set[str] = set()

    def walk(node: object) -> None:
        if isinstance(node, dict):
            for key, value in node.items():
                if isinstance(key, str) and _keep_term(key):
                    terms.add(key.strip())
                walk(value)
