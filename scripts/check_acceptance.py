...
EXPECTED_IDS: dict[str, str] = {
    ...
    "A84": "v0.7",  # 批 4：档案图库（挂图/重复挂 409/撤图/上传图）
    "A85": "v0.7",  # 批 4：图足迹软提示（登记/查询/幂等/撤销）
    "A86": "v0.7",  # 批 5：写接口唯一通道 + 共享 service（代码级断言）
    "A87": "v0.7",  # 批 5：建档页/素材库联动（页面渲染 + DOM 断言）
    "A88": "v0.7",  # 批 5：引擎零改动回归（registry-check + 文件树）
    "A89": "v0.7",  # 批 2：存量接缝（ERP 编号+无图建档）
    "A90": "v0.7",  # 复核修订：编号自动生成（§16.2）
    "A91": "v0.7",  # 复核修订：迁移 0014 + specs 校验（§16.3）
    "A92": "v0.7",  # 批 2 页面层：编辑功能（§16.4）
    "A93": "v0.7",  # 批 2 页面层：建档页去库存（§16.5 M18）
    "A94": "v0.7",  # 批 2 页面层：图库文件夹浏览器（§16.6）
    "A95": "v0.7",  # 批 2 页面层：导航重构（§16.5）
}
...
def check_manifest(
    ...
    if missing:
        ...
        red.append(
            ...
            f"（共 {len(missing)} 个，需覆盖 A1-A89 全集）"
        )
